...
for (int i = 1; i <= ProviderPayRequest.getRowNumbers(); i++) {
    ...
    expectedErrorMSG = (((('Pay Request already exists on service date ' + ProviderPayRequest.getValue('RequestDate', i)) + 
    ' for pay element ') + ProviderPayRequest.getValue('PayElement', i)) + ' , this request will not be raised')
