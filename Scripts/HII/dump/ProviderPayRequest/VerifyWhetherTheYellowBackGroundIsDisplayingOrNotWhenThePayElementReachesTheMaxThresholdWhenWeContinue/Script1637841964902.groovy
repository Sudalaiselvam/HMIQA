...
for (int i = 1; i <= ProviderPayRequest.getRowNumbers(); i++) {
    ContractPayElementThreshold = ProviderPayRequestSteps.getPayElementThreshold(ProviderPayRequest.getValue('ProviderName', 
            i), ProviderPayRequest.getValue('RequestDate', i), ProviderPayRequest.getValue('CompensationModel', i), ProviderPayRequest.getValue(
            'PayElement', i))
    ...
    ProviderPayRequestSteps.savePayRequest_reachingMaxThresholdContinue(ProviderPayRequest.getValue('RequestDate', i), ProviderPayRequest.getValue(
            'CompensationModel', i), ProviderPayRequest.getValue('PayElement', i), ProviderPayRequest.getValue('PayElementValue', 
            i),expectedSuccessMSG,expectedErrorMSG)
}