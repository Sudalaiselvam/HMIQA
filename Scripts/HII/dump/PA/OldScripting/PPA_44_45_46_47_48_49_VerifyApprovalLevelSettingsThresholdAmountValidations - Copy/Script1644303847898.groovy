...
for (int i = 1; i <= ProviderPayApprovals.getRowNumbers(); i++) {
	...
	String ProviderName = ProviderPayApprovals.getValue('ProviderName', i)
	...
	String PayRequestPayElement = ProviderPayApprovals.getValue('PayRequestPayElement', i)

	'Step 2: Login with Compensation Administrator'
	LoginPageSteps.Login(GlobalVariable.COMPADMIN_USER, GlobalVariable.COMPADMIN_PWD, HIIConstants.ROLE_CompensationAdministrator)

	
	ManageProviderContractSteps.getContractPayElement(index)
	
	'Step 3: Create New Provider Pay Request'
	ProviderPayRequestSteps.createProviderRequestwithCurrentDate(ProviderName, ServiceDate, CompensationModel, PayRequestPayElement,PayElementValue)
