...
for (int i = 1; i <= ProviderPayRequest.getRowNumbers(); i++) {
	...
	ContractMaxCompensation=ProviderPayRequestSteps.getMasterContractMaxCompensation(ProviderPayRequest.getValue('ProviderName', i), ProviderPayRequest.getValue('RequestDate', i), ProviderPayRequest.getValue('CompensationModel', i), HIIConstants.MaxCompensationDuration_WithintheContract)
	...
	ProviderPayRequestSteps.savePayRequest_ContinueExceedingMaxCompensation_WithintheContract(ProviderPayRequest.getValue('RequestDate', i), ProviderPayRequest.getValue('CompensationModel', i), ProviderPayRequest.getValue('PayElementValue',i), ContractMaxCompensation)
